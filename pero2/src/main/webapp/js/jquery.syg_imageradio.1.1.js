...
( function( jQuery ){
	...
	$.sygImageRadio = function( selector, config ){
		...
		function setRadioButton( target ){
			...
			// オーバー画像と選択画像
			target.imgsrc_over = target.imgsrc.substr(0,dotpos) + '_over.' + target.imgsrc.substr(dotpos+1);
			target.imgsrc_select = target.imgsrc.substr(0,dotpos) + '_select.' + target.imgsrc.substr(dotpos+1);
